...
- **DB구성** : 유저 테이블, 키워드 테이블, 관심분야 테이블, 알림받을 매장 테이블 2가지로 구성
  ...
  - ~~추후 관심분야 테이블 추가 예정~~
      ...
      - 관심분야 테이블 및 알림받을 매장 테이블 추가
          - 관심 분야 테이블 : 관심있는 책의 장르 다중 선택 가능하게 함
          - 알림받을 매장 테이블 : 사용자가 알림을 받을 매장을 다중 선택 가능하게 함
...
- ~~유저가 9 ~ 10개의 키워드를 입력한 경우. '로그인'이나 '메인으로'를 클릭하여 검색 페이지로 넘어가면서 30초 이상 걸려 heroku에서 H12에러를 발생시키는 현상. 크롤링을 하는 데 30초 내외가 걸리기 때문에 서버 상태에 따라 에러가 발생할 때도 있고 아닐 때도 있음~~ 등록할 수 있는 키워드의 갯수를 8개로 조정하여 서버에러 발생 확률을 줄임. 다만 8개 미만일 때에도 서버의 반응 속도에 따라 에러가 발생하기도 함.
...
- ~~회원가입 시 관심분야를 수집할 수 있도록 하고 DB에 저장~~(완료)
  - ~~회원 가입 페이지에서 관심 분야를 중복 선택할 수 있도록 설정~~(완료)
  - ~~관심분야 테이블을 따로 만들어 유저의 관심 분야를 저장하도록 함~~(완료)
...
- Main Page(데스크탑 화면)  
  ...
  ![image](https://user-images.githubusercontent.com/52812181/89160159-99855f00-d5ab-11ea-8677-e0190c92dfe3.png)

<br/>

- Main Page(모바일 화면)  

  <img src="https://user-images.githubusercontent.com/52812181/89160654-5bd50600-d5ac-11ea-8e96-d1178aeccd0e.jpeg" width="380" height="600">
...
- Store Page(데스크탑 화면)
    ...
    ![](https://user-images.githubusercontent.com/52812181/89160866-b53d3500-d5ac-11ea-95f9-fec3edfab1e3.png)

<br/>

- Store Page(모바일 화면)

    <img src="https://user-images.githubusercontent.com/52812181/89161009-f46b8600-d5ac-11ea-89ce-d428b6bc3598.jpeg" width="350" height="600">
...
- 회원가입 페이지에 추가된 관심 장르 및 알림받을 매장 선택 부분

    ![image](https://user-images.githubusercontent.com/52812181/89159843-0cdaa100-d5ab-11ea-920f-d429a5245971.png)
