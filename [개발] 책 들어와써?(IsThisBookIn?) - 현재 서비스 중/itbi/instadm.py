...
# instagram api / apscheduler load
...
if __name__ == '__main__':
    ...
    cur = sql.connect_sql() # mysql server connect
    users = sql.bring_user(cur) # user data select
    keyword = sql.bring_keyword(cur) # keyword data select
    user_dict = sql.dict_users(users, keyword) # dictionary combine
    instadm = instaDM(user_dict) # instaDM 함수 할당

    sched.add_job(
        instadm.crawl_and_dm(), # 크롤링 및 다이렉트 메세지 전송
        'cron',
        hour=9,		# 오전9시30분
        minute=30
    )
